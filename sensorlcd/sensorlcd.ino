...
int counter = 0;
float firstAccel = 0;
int linearvelocidade = 0;

//Interrupt sensor
const int sensorA = 3; //digital pin where sensor is connected
unsigned int rpmA;     // rpm reading
volatile int pulsesA = 0;  // number of pulses
double velocidade = 0;
unsigned int pulsesperturn = 32; // number of pulses per revolution


void counterA()
{
    pulsesA++; //update pulses
...
void setup()
{
    ...
    pinMode(sensorA, INPUT); // Use statusPin to flash along with interrupts
    //Interrupt 0 is digital pin 2, so that is where the IR detector is connected
    //Triggers on FALLING (change from HIGH to LOW)
    attachInterrupt(1, counterA, FALLING);
    // Initialize
    pulsesA = 0;
    rpmA = 0;
    timeold = 0;
...
//  accel_x = (float) sqrt(sq(accel_t_gyro.value.y_accel*(9.81 / 16384))+sq(accel_t_gyro.value.z_accel*(9.81 / 16384))) - firstAccel;
  ...
  if( firstAccel == 0 ){
   ...
   if (millis() - timeold >= 250){  /*Uptade every one second, this will be equal to reading frecuency (Hz).*/
      ...
      velocidade = ((2.1/32) * pulsesA) / (millis() - timeold)*1000; // velocidade = ((tireLength / pulsesPerRevolution) * pulses) / (timeElapsed in sec)

...
//pushbutton for activating the abs
int inPin = 7;
int break_pressed = 0;
...
//  Serial.println(error,DEC);
  ...
  break_pressed = digitalRead(inPin);  // read input value



  float accel_x;
  if(break_pressed == HIGH){
      accel_x = (float) (accel_t_gyro.value.x_accel * 9.81 / 16384) - firstAccel;
      Serial.print(F("Aceleracao eixo X: "));
  ...
  Serial.print(F("Aceleracao media X: "));
  Serial.println(meanAccel, 4);
  Serial.println("");
  ...
  if (millis() - timeold >= 1000){
    ...
    //linearspeed = rpm * wheel length / 60
    linearspeed = ((60 * 1000 / pulsesperturn )/ (millis() - timeold)*pulses)*(2*3.1415*radius)/(60);
    pulses = 0;
    meanSpeed = linearSpeed;
    ...
    Serial.println(meanSpeed, 4);
    Serial.println("");
    
    //attaching back interrupts
    attachInterrupt(0, counter, FALLING);
  }
